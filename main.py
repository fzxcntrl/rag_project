...
def get_vector_store():
    global vector_store
    if vector_store is None and FAISS_DIR.exists() and any(FAISS_DIR.iterdir()):
...
def process_upload(file_path: Path):
    global vector_store
...
async def lifespan(app: FastAPI):
    await run_in_threadpool(get_embeddings)
    get_vector_store()
    yield
...
async def upload_document(file: UploadFile = File(...)):
    ...
    try:
        ...
        await run_in_threadpool(process_upload, file_path)
        return {"message": f"Successfully uploaded and indexed '{safe_name}'."}
    except Exception as e:
        logger.exception("Error processing upload for %s", safe_name)
